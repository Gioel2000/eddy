...
export class Step1CompetitorDialog {
  ...
  constructor() {
    ...
    toObservable(this.store.selected)
      ...
      .subscribe((selected) => {
        ...
        const lat = selected.geometry?.location?.lat || 0;
        const lng = selected.geometry?.location?.lng || 0;
        const name = selected.name || '';
        const address: string =
          selected?.address_components?.find(
            (c) => c.types.includes('route') || c.types.includes('sublocality_level_2')
          )?.long_name || '';
        const number: string =
          selected?.address_components?.find((c) => c.types.includes('street_number'))?.long_name || '';
        const zipCode: string =
          selected?.address_components?.find((c) => c.types.includes('postal_code'))?.long_name || '';
        const city: string =
          selected?.address_components?.find(
            (c) =>
        ...
        const telephone = selected.phone || '';
        const website = selected.website || '';
        const email = '';
        const googlePlaceId = selected.place_id || '';
        const googleMapsLink = selected.url || '';
        ...
        this.photos.set(selected.photos.map((photo) => this.getUrl(photo)).map((photo) => ({ preview: photo })));
        this.selectedType.set(0);
        this.googlePlaceId.set(googlePlaceId);
        this.googleLink.set(googleMapsLink);
  ...
  add() {
    const { name, address, zipCode, city, telephone, website, email } = this.formGroup.getRawValue();
