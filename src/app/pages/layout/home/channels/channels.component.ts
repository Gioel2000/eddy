...
export class ChannelsDropdownComponent {
  ...
  checkedChannels = computed(() => this.home.filter().channels.join(', '));

  thereIsTheFork = computed(() => this.home.filter().channels.includes('thefork'));
  thereIsGoogle = computed(() => this.home.filter().channels.includes('google'));
  thereIsTripAdvisor = computed(() => this.home.filter().channels.includes('tripadvisor'));

  toggle(service: string) {
    const channels = this.home.filter().channels;
    const thereIsService = channels.includes(service);

    if (thereIsService) {
      this.home.filter.set({
        ...this.home.filter(),
