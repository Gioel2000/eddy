...
@Component({
  ...
  template: `
    ...
    >
      <div>
        <div
          class="h-44 sm:h-52 md:h-60 w-56 sm:w-60 md:w-72 overflow-hidden rounded-t-2xl bg-zinc-200 dark:bg-zinc-900 group-hover:opacity-75"
        >
          @if (dish().dish.image) {
          <div class="relative flex h-full w-full flex-col overflow-hidden rounded-t-2xl p-6">
      ...
      <div class="flex flex-col gap-y-2 p-3 mb-1">
        @if (peanut()) {
        <div class="flex flex-row gap-x-1">
          <span class="svg-icon-5 text-amber-500 stroke-2">
        ...
        <div class="flex flex-row gap-x-1">
          <span class="svg-icon-5 text-sky-500 stroke-2">
          ...
          <span class="text-sky-500 text-sm font-medium">
            {{ 'MILK' | translate }}
          </span>
        </div>
        }@if (gluten()) {
        <div class="flex flex-row gap-x-1">
          <span class="svg-icon-5 text-emerald-500 stroke-2">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
              <path
                fill="currentColor"
