...
@UntilDestroy()
@Component({
  selector: 'structure-dropdown-sm',
  standalone: true,
  imports: [
    CommonModule,
    InlineSVGModule,
  ...
  template: `
    ...
    <div [ngClass]="{ hidden: !ui.isDropdownOpenSm() }">
      ...
      >
        ...
        <ul class="z-10 h-44 w-full overflow-y-auto py-1 text-base focus:outline-none sm:text-sm">
          ...
          <li
            class="text-zinc-600 dark:text-zinc-400 relative cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-900 select-none py-2 pl-3 pr-9"
            id="listbox-option-0"
            role="option"
            (click)="structures.choose(restaurant._id); layout.closeMenu()"
          >
        ...
        <div class="py-2" role="none">
          ...
          <a
            ...
            tabindex="-1"
            id="menu-item-0"
            (click)="structures.exit(); restaurantPanel.closePanel(); layout.closeMenu()"
          >
            <div class="flex flex-row items-center w-full">
              <span
    ...
    <button
      type="button"
      class="flex items-center p-2 hover:bg-zinc-600/5 dark:hover:bg-zinc-300/5 rounded-lg w-full"
      id="user-menu-button"
      aria-expanded="false"
      aria-haspopup="true"
      [ngClass]="{
        'bg-zinc-600/5 dark:bg-zinc-300/5': ui.isDropdownOpenSm(),
      }"
      (click)="ui.toggleDropdownSm()"
      (clickOutside)="isSearchBlur() && ui.closeDropdownSm()"
    >
...
export class StructureSmComponent {
